		import { handleMeshClick } from './modules/editmaterial.js';

//		import { exportViewer } from './modules/exportViewer.js';
        ...
        const controlSettings = {
			...
			publishModel: async function () {
				...
				script.src = 'js/modules/exportViewer.js'; // đúng đường dẫn bạn lưu file
...
//			loadAnnotationsFromFileIfExists();
        ...
        function initEditorModeControls() {
			try {
				...
				.addButton('📤Export Viewer', controlSettings.publishModel, {
					label: '📤Export Viewer'
				})
...
function loadAnnotationsFromFileIfExists() {
			fetch('./default.json')
				...
				.then(data => {
					...
					window.annotationArray = annotations;
...
async function publishModel() {
  const glbBlob = await exportToBlob(scene); // hoặc lấy từ GLTFExporter của bạn
  const annotations = yourAnnotationArray; // thay bằng biến thật sự đang chứa annotations

  await exportViewer(scene, annotations, glbBlob, {
    modelFileName: 'model.glb',
    viewerTitle: 'Máy bán vé tự động',
  });
}