	//	import { handleMeshClick } from './main.module.js';
        ...
        function onSingleClick(event) {
			...
			// Kiểm tra xem có click vào vật thể 3D không
			raycaster.layers.set(0);
			const intersectsMeshes = raycaster.intersectObject(currentModel, true);
			if (intersectsMeshes.length > 0) {
				const intersect = intersectsMeshes[0];
				const mesh = intersect.object;
				const mat = mesh.material;

				if (mat && mat.isMeshStandardMaterial) {
					// Log vị trí click ra console
					console.log("Clicked Mesh Position:", intersect.point);

					// Gán các input trong panel
					...
					const envmapInput = document.getElementById('material-envmap');
					...
					envmapInput.value = mat.envMapIntensity ?? 1;
					...
					envmapInput.oninput = () => { mat.envMapIntensity = parseFloat(envmapInput.value); mat.needsUpdate = true; };
					transparentInput.onchange = () => { mat.transparent = transparentInput.checked; mat.needsUpdate = true; };

					document.getElementById('material-editor-panel').style.display = 'block';
				}
			}
